...
namespace SotCore.Util
{
    public class Memory : IDisposable
    {
        ...
        public void Dispose()
        {
            foreach (var kvp in _allocations) VirtualFreeEx(procHandle, kvp.Key, kvp.Value, 0x4?0);
        }
        ...
        {
            //var retValPtr = VirtualAllocEx(procHandle, IntPtr.Zero, 0x40, 0x1?0, 0x40);
            ...
            var dummyParms = VirtualAllocEx(procHandle, IntPtr.Zero, 0x1?, 0x1?0, 0x40);
            _allocations.Add(dummyParms, 0x1?);
            ...
            asm.Add(0xC3); // ret
            var codePtr = VirtualAllocEx(procHandle, IntPtr.Zero, asm.Count, 0x1?0, 0x40);
        ...
        public List<UInt64> SearchProcessMemory(String pattern, UInt64 start, UInt64 end, Boolean absolute = true)
        {
            ...
            var iters = 1 + ((end - start) / 0x1?0);
            if (iters == 0) iters++;
            for (uint i = 0; i < iters; i++)
            {
                var buffer = new Byte[0x1?0];
                ReadProcessMemory(procHandle, (UInt32)(start + i * 0x1?0), buffer, 0x1?0, out Int32 bytesRead);
                var results = Scan(buffer, arrayOfBytes).Select(j => (UInt64)j + start + i * 0x1?0).ToList();
                if (start + (i + 1) * 0x1?0 > end && results.Count > 0)
