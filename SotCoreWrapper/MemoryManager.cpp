...
cMemoryManager::~cMemoryManager()
{
	CloseHandle(Core::memoryProcess);
}
...
uintptr_t cMemoryManager::FindSignature(uintptr_t base, uintptr_t size, BYTE* sign, char* mask)
{
	...
	while (offset < size)
	{
		VirtualQueryEx(Core::memoryProcess, (LPCVOID)(base + offset), &mbi, sizeof(MEMORY_BASIC_INFORMATION));
		if (mbi.State != MEM_FREE)
		{
			BYTE* buffer = new BYTE[mbi.RegionSize];
			ReadProcessMemory(Core::memoryProcess, mbi.BaseAddress, buffer, mbi.RegionSize, NULL);
...
uintptr_t cMemoryManager::baseModuleAddress(const char* ModuleName)
{
	...
	HANDLE hSnap = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE | TH32CS_SNAPMODULE32, Core::memoryProcessID);
...
{
	HANDLE hModule = CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, Core::memoryProcessID);
